...
function MetricCard({
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-2xl border bg-card/40 p-5",
...
function ListenDailyChart({
  ...
  const maxVal = useMemo(
    () => Math.max(1, ...filled.map((x) => x.full + x.partial)),
    [filled],
  );
  ...
  return (
    <div className="w-full overflow-x-auto">
      ...
      >
        <defs>
          <linearGradient id="barFull" x1="0" y1="1" x2="0" y2="0">
            <stop offset="0%" stopColor="rgb(16 185 129)" stopOpacity="0.35" />
            <stop offset="100%" stopColor="rgb(16 185 129)" stopOpacity="0.95" />
          </linearGradient>
          <linearGradient id="barPart" x1="0" y1="1" x2="0" y2="0">
        ...
        return (
          <div key={t.id} className="space-y-1">
            ...
            <div className="h-2 rounded-full bg-muted overflow-hidden">
              <div
                className="h-full rounded-full bg-gradient-to-r from-emerald-600/80 to-emerald-400/90"
                style={{ width: `${pct}%` }}
              />
            </div>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                Як слухаюць вашы трэкі на платформе
              </p>
            </div>
          </div>
        </header>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5 max-w-xl">
              Прагляд праслухоўванняў па днях (часавы пояс чарта — Мінск), унікальныя слухачы і зводка па кожным
              трэку. Поўнае праслухоўванне — без скарачэнняў да канца трэка ў межах правіл анты-накруткі.
            </p>
          </div>
        </div>
      ...
      <div className="glass rounded-2xl border border-border overflow-hidden">
        <div className="px-5 py-4 border-b border-border/60 bg-card/20">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            За перыяд і ўсяго (сума поўных і частковых сесій)
          </p>
        </div>
                    ...
                    <tr key={t.id} className="border-b border-border/40 last:border-0 hover:bg-muted/30">
                      ...
                      <td className="px-4 py-3">
                        {href ? (
                          <Link
                            href={href}
                            className="inline-flex text-muted-foreground hover:text-emerald-600 dark:hover:text-emerald-400"
