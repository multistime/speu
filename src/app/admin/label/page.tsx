...
function AdminLabelPageInner() {
  ...
  const tab: "songs" | "albums" | "artists" =
    rawTab === "albums" || rawTab === "artists" ? rawTab : "songs";

  const navigateToTab = (next: "songs" | "albums" | "artists") => {
    const q = next === "songs" ? "" : `?tab=${next}`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass rounded-2xl border border-border p-6">
        <h1 className="font-display text-2xl italic text-foreground mb-2">Лэйбл</h1>
        <p className="text-sm text-muted-foreground">
          Песні, альбомы і артысты — каталог музычнага лэйбла.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          type="button"
          onClick={() => {
            navigateToTab("artists");
            setError(null);
            setSongFormOpen(false);
            setAlbumFormOpen(false);
          }}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === "artists" ? "bg-primary text-primary-foreground" : "bg-muted text-foreground/70 hover:text-foreground"}`}
        >
          <Users className="w-4 h-4" /> Артысты
        </button>
      </div>
