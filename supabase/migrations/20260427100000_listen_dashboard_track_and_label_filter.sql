-- Per-track listen series (cabinet + admin) and label overview: optional artist filter + top artists.
...
comment on function speu.track_listen_dashboard(uuid, integer) is
  'Listen daily series for one track. Allowed for admins or artists linked to the track.';
...
-- ---- label_listen_dashboard: optional artist filter + artists leaderboard ----
drop function if exists speu.label_listen_dashboard(integer);
...
comment on function speu.label_listen_dashboard(integer, uuid) is
  'Admin: listen KPIs, daily series, per-track totals; optional p_filter_artist_id narrows scope. Artists leaderboard is always label-wide for the period.';
