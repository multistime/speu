-- Listen dashboards: per-track flag analytics_eligible (same semantics as record_listen_terminal gate:
-- speu.track_is_publicly_likeable). Product choice: Variant A — counts only apply to eligible tracks;
-- ineligible tracks show zeros because listens are not recorded (see methodology in app).

-- ---- track_listen_dashboard ---------------------------------------------------
create or replace function speu.track_listen_dashboard(
  p_track_id uuid,
...
begin
  ...
  select
    case
      when btrim(coalesce(t.title, '')) = '' then 'Без назвы'
      else t.title
    end
  into v_title
  ...
  if not speu.is_admin(v_uid) and not exists (
    select 1
    from speu.artist_tracks t
    where t.id = p_track_id
      and speu.user_owns_artist(t.artist_id)
    union all
    select 1
    from speu.track_artists ta
    where ta.track_id = p_track_id
      and speu.user_owns_artist(ta.artist_id)
  ) then
  ...
  with daily as (
    select
      ...
      coalesce(l.full_total, 0)::bigint as full_c,
      coalesce(l.partial_total, 0)::bigint as part_c
    from speu.track_listen_by_day_minsk l
    where l.track_id = p_track_id
      and l.day_minsk between v_start and v_end
  ),
  daily_agg as (
    select coalesce(
      jsonb_agg(
  ...
  select jsonb_build_object(
    ...
    'track', jsonb_build_object(
      'id', p_track_id,
      'title', v_title,
      'analytics_eligible', coalesce(speu.track_is_publicly_likeable(p_track_id), false)
    )
  )
  into r;
...
comment on function speu.track_listen_dashboard(uuid, integer) is
  'Listen daily series for one track. track.analytics_eligible matches record_listen_terminal gate.';

revoke all on function speu.track_listen_dashboard(uuid, integer) from public;
grant execute on function speu.track_listen_dashboard(uuid, integer) to authenticated;

-- ---- artist_listen_dashboard (cabinet) ----------------------------------------
    ...
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'analytics_eligible', coalesce(speu.track_is_publicly_likeable(tb.id), false)
        )
...
comment on function speu.artist_listen_dashboard(integer) is
  'Cabinet: listen KPIs; per-track analytics_eligible matches record_listen_terminal gate.';
...
-- ---- label_listen_dashboard ---------------------------------------------------
create or replace function speu.label_listen_dashboard(
  p_period_days integer default 28,
  p_filter_artist_id uuid default null
)
returns jsonb
language plpgsql
volatile
...
begin
  ...
  with my_tracks as (
    select t.id as track_id
    from speu.artist_tracks t
    where p_filter_artist_id is null
      or t.artist_id = p_filter_artist_id
      or exists (
        select 1
        from speu.track_artists ta
        where ta.track_id = t.id
          and ta.artist_id = p_filter_artist_id
      )
  ),
  daily as (
    select
  ...
  period_listens_by_track as (
    select
      l.track_id,
      (sum(l.full_total) + sum(l.partial_total))::bigint as sess
    from speu.track_listen_by_day_minsk l
    where l.day_minsk between v_start and v_end
    group by l.track_id
  ),
  artist_listen_totals as (
    select
      t.artist_id,
      sum(coalesce(p.sess, 0))::bigint as period_sessions
    from speu.artist_tracks t
    left join period_listens_by_track p on p.track_id = t.id
    group by t.artist_id
  ),
  artists_ranked as (
    select a.id as artist_id, a.name, alt.period_sessions
    from artist_listen_totals alt
    inner join speu.artists a on a.id = alt.artist_id
    where alt.period_sessions > 0
    order by alt.period_sessions desc, a.name
    limit 50
  ),
  artists_agg as (
    select coalesce(
      jsonb_agg(
        ...
        order by period_sessions desc, name
      ),
      '[]'::jsonb
    ) as j
    from artists_ranked
  ...
  select jsonb_build_object(
    'ok', true,
    'period_days', v_days,
    'filter_artist_id', p_filter_artist_id,
    'range', jsonb_build_object('start', v_start, 'end', v_end),
    'prev_range', jsonb_build_object('start', v_prev_start, 'end', v_prev_end),
    'summary', jsonb_build_object(
      'full_listens', (select full_c from summary_now),
    ...
    'daily', (select j from daily_agg),
    'tracks', (select j from tracks_agg),
    'artists', (select j from artists_agg),
...
comment on function speu.label_listen_dashboard(integer, uuid) is
  'Admin: listen KPIs; per-track analytics_eligible matches record_listen_terminal gate.';

revoke all on function speu.label_listen_dashboard(integer, uuid) from public;
grant execute on function speu.label_listen_dashboard(integer, uuid) to authenticated;
